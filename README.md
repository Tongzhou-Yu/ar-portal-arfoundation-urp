...
2. Adding `--share` to the command line options in `webui-user.bat` will generate a random share link that is displayed in the command window, that you can fill in `DomeTransition_RuntimeSD.csharp`'s `Url`.  
